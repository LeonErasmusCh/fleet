...
@api.route('/encomiendas', methods=['GET', 'POST'])
def all_encomiendas():
    if request.method =='GET':
...
def post_encomiendas():    
    ...
    newEncomiendas= Encomiendas (status=body['status'], originAddress=body['originAddress'], destinationAddress=body['destinationAddress'], mensaje=body['mensaje'])
